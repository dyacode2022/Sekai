  ...
  if (response[i].id == window.sessionStorage.getItem("usrId")) {
    ...
    for (let j = 0; j < response[i].watched.length; j++) {
      es.push(<div key={j} ind={j} className='card w-64 h-56 bg-slate-50 drop-shadow-xl rounded-3xl m-20 text-center hover:scale-110 hover:-translate-y-1 duration-100'>
        <img className='cardImg w-64 h-40 object-cover rounded-2xl mb-5' src={ response[i].watched[j].img } />
        <div className='cardTitle'>{ response[i].watched[j].title }</div>
      </div>)
    }
...
export default function UsrComponent() {

  return (
    <div className="UsrComponent font-mono text-center mb-52">

        <div className='relative popularity z-10 mb-20 font-semibold text-3xl text-center mt-20 text-transparent bg-clip-text bg-gradient-to-r from-gradient-blue to-gradient-purple'>정주행한 작품</div>
